...
module.exports = class authToken {
    ...
    async save() {
        try {
            const db = getDB('algotrade');

            const lastData = await db.collection('authToken').find({}).sort({ _id: -1 }).limit(1).next(); //get last id
            ...
            const result = await db.collection('authToken').insertOne(this)
            return result;
